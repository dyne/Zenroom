...
class Bip340Witness {
  ...
  /* ---- Private: fields, SHA construction, scalar-mul ---- */
  const Field& f_;
  const EC& ec_;
  const ScalarField& fn_;

  /* SHA data (public for test access) */
 public:
  ...
  bool build_scalar_mul(const Elt pk_x, const Elt pk_y,
    ...
    for (size_t i = 0; i < kBits; ++i) {
      ...
      bi_[i] = f_.subf(f_.of_scalar(2 * bi), f_.of_scalar(7));  /* center: -7..7 */
      Point mux;
      switch (bi) {
        case 0: mux = ec_.zero(); break;
        case 1: mux = G;  break; case 2: mux = P;  break;
        case 3: mux = GP; break; case 4: mux = R;  break;
        case 5: mux = GR; break; case 6: mux = PR; break;
        case 7: mux = GPR; break; default: return false;
      }
      if (i > 0) ec_.doubleE(ax, ay, az, ax, ay, az);
      ec_.addE(ax, ay, az, ax, ay, az, mux.x, mux.y, mux.z);
      if (i < kBits - 1) { int_x_[i] = ax; int_y_[i] = ay; int_z_[i] = az; }
    }
    return ax == f_.zero() && az == f_.zero();
  }

  /* ---- Public: witness output fields ---- */
  Elt rx_, ry_, s_inv_, pk_inv_;
  Elt pre_[8];
  Elt bi_[kBits];
  Elt int_x_[kBits - 1], int_y_[kBits - 1], int_z_[kBits - 1];
  Elt e_challenge_, e_neg_, c_val_;
  ...
  bool compute(const uint8_t pk_bytes[32], const uint8_t R_bytes[32],
    ...
    }
    {
      ...
      auto e_neg_n = fn_.subf(n_val, e_n);
      auto c_n     = fn_.subf(n_val, fn_.one());
    ...
    auto elt_to_bits = [&](const Elt& e, Elt* out) {
      nat_to_bits(f_.from_montgomery(e), out);
    };
    auto low8 = [&](const Elt& e, Elt* out) {
      Nat n = f_.from_montgomery(e);
      for (size_t i = 0; i < 8; ++i) out[i] = f_.of_scalar(n.bit(i) ? 1 : 0);
    };
  ...
  void fill_witness(proofs::DenseFiller<Field>& filler) const {
    filler.push_back(rx_); filler.push_back(ry_);
    filler.push_back(s_inv_); filler.push_back(pk_inv_);
    for (size_t i = 0; i < 8; ++i) filler.push_back(pre_[i]);
    for (size_t i = 0; i < kBits; ++i) {
      filler.push_back(bi_[i]);
      if (i < kBits - 1) {
        filler.push_back(int_x_[i]); filler.push_back(int_y_[i]);
        filler.push_back(int_z_[i]);
      }
    }
    filler.push_back(e_challenge_); filler.push_back(e_neg_);
