...
covers and what must be built under `lib/niwi` to use native secp256k1
...
## What must be built under lib/niwi
...
files under `lib/niwi` and one set of hardcoded FFT root constants.

## Build integration

New files under `lib/niwi/src/secp256k1/`:

```
lib/niwi/src/secp256k1/
...
Added to `lib/niwi/sources.mk` and compiled by the existing `lib/niwi/GNUmakefile`.
