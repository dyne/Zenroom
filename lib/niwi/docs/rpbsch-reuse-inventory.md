...
| **Adapt** | Copy under `lib/niwi` with provenance, no upstream changes |
| **New**   | Implement under `lib/niwi` from scratch              |
...
| `FpSecp256k1Base`             | **New**   | `lib/niwi/src/secp256k1/secp256k1_field.*`    | secp256k1 base field instance |
| `FpSecp256k1Scalar`           | **New**   | `lib/niwi/src/secp256k1/secp256k1_scalar.*`   | secp256k1 scalar field (mod n) |
| `EllipticCurve<F, W, kN>`     | **Reuse** | `lib/longfellow-zk/ec/elliptic_curve.h`       | Generic EC template |
| `Secp256k1` (curve instance)  | **New**   | `lib/niwi/src/secp256k1/secp256k1_curve.*`    | secp256k1 curve instance |
...
| `LigeroProver<F, IF>`           | **Adapt** | `lib/niwi/ligero/niwi_ligero_exposed.h`       | Already adapted in NIWI milestone |
| `LigeroVerifier<F, IF>`         | **Adapt** | `lib/niwi/ligero/`                            | Already adapted |
| `LigeroTranscript<F>`           | **Adapt** | `lib/niwi/ligero/`                            | Already adapted |
| `MerkleCommitment`, `MerkleTree`| **Adapt** | `lib/niwi/ligero/`                            | Already adapted |
...
| Component                       | Class     | Location                                      | Notes |
|---------------------------------|-----------|-----------------------------------------------|-------|
| `niwi_ctx_t`                    | **Reuse** | `lib/niwi/include/niwi.h`                     | Already built |
| `niwi_prove` / `niwi_verify`    | **Reuse** | `lib/niwi/src/niwi.c`                         | Already built |
| `niwi_klp22_commit` / `_verify` | **Reuse** | `lib/niwi/src/commitment.c`                   | Already built |
| `niwi_leaf_commit` / `_verify`  | **Reuse** | `lib/niwi/src/commitment.c`                   | Already built |
| `niwi_npro_t`                   | **Reuse** | `lib/niwi/src/npro.c`                         | Already built (test-only) |
| `niwi_prove_observed`           | **Reuse** | `lib/niwi/src/niwi.c`                         | Already built (test-only) |
| `niwi_extract`                  | **Reuse** | `lib/niwi/src/extract.c`                      | Already built (test-only) |

## RPBSch-specific: New components

| Component                       | Class     | Location                                      | Notes |
|---------------------------------|-----------|-----------------------------------------------|-------|
| BIP-340 verification circuit    | **New**   | `lib/niwi/src/circuits/bip340_circuit.*`      | Standalone reusable circuit |
| BIP-340 witness generator       | **New**   | `lib/niwi/src/circuits/bip340_witness.*`      | Witness from byte-level BIP-340 sigs |
| Pedersen commitment circuit     | **New**   | `lib/niwi/src/circuits/pedersen_circuit.*`    | Cmt equation in-circuit |
| RPBSch branch-1 circuit         | **New**   | `lib/niwi/src/circuits/rpbsch_circuit.*`      | Honest-user branch |
| RPBSch branch-2 circuit         | **New**   | `lib/niwi/src/circuits/rpbsch_circuit.*`      | Trapdoor branch (same file) |
| RPBSch OR composition           | **New**   | `lib/niwi/src/circuits/rpbsch_circuit.*`      | Private selector |
| RPBSch witness/stmt builder     | **New**   | `lib/niwi/src/rpbsch_witness.*`               | Construct witnesses from scalars |
| RPBSch statement serializer     | **New**   | `lib/niwi/src/rpbsch_stmt.*`                  | Byte-level statement encoding |
| PBSch protocol state machine    | **New**   | `lib/niwi/src/pbsch.*`                        | Figure 4 protocol steps |
| PBSch Cmt (prototype)           | **New**   | `lib/niwi/src/pbsch_commitment.*`             | Pedersen + hash opening proof |
| PBSch Lua bindings              | **New**   | `lib/niwi/src/pbsch_lua_bindings.c`           | Thin Lua wrappers |
...
- `lib/niwi/ligero/` (adaptation is frozen as-is)
