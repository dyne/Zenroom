...
@test "DCQL: simple dc+sd-jwt query" {
    cat << EOF | save_asset simple_dcsdjwt_dcql_query.keys.json
{
    "my_query": {
        "credentials":  [
            {
...
EOF
    cat << EOF | save_asset simple_dcsdjwt_dcql_query.data.json
{
    "credentials_list": {
        "dc+sd-jwt": [
...
EOF
    cat << EOF | zexe simple_dcsdjwt_dcql_query.zen simple_dcsdjwt_dcql_query.data.json simple_dcsdjwt_dcql_query.keys.json
...
EOF
    save_output credentials_list.out.json
    assert_output '{"credentials":[{"matching_credential_sets":[{"example_sd_jwt":["eyJhbGciOiAiRVMyNTYiLCAidHlwIjogImRjK3NkLWp3dCJ9..oREhEn5mspszmYxJOYAyWV8WoAsC4A6ymf-TWEBhXAokFGYmgd0doIJZvbVdYKyRkuvte6dNJnbymhN5xUhdlQ~WyJFSmw3N2lEa1Mzc2tIQURPaTFSN1VBIiwgImhhc19kaXNjb3VudF9mcm9tX3ZvdWNoZXIiLCAxMF0~"]}],"required":true}]}'
}

@test "DCQL: simple ldp_vc query" {
    cat << EOF | save_asset simple_ldpvc_dcql_query.keys.json
{
    "my_query": {
        "credentials":  [
            {
...
EOF
    cat << EOF | save_asset simple_ldpvc_dcql_query.data.json
{
    "credentials_list": {
        "ldp_vc": [
            {
...
EOF
    cat << EOF | zexe simple_ldpvc_dcql_query.zen simple_ldpvc_dcql_query.data.json simple_ldpvc_dcql_query.keys.json
Scenario 'dcql_query': test
Given I have a 'dcql_query' named 'my_query'
Given I have a 'string dictionary' named 'credentials_list'
When create the credentials from 'credentials_list' matching the dcql_query 'my_query'
...
EOF
    cat << EOF | zexe dcql_query.zen dcql_query.data.json dcql_query.keys.json
Scenario 'dcql_query': test
Given I have a 'dcql_query' named 'my_query'
Given I have a 'string dictionary' named 'credentials_list'
When create the credentials from 'credentials_list' matching the dcql_query 'my_query'
