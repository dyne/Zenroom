...
#num_1 into the dictionaries, num_3 out of the dictionaries 
...
@test "error" {
    cat <<EOF | save_asset when_error.data.json
{
	"string": "there is an error!"
}
EOF
    cat <<EOF | save_asset when_error.zen
Given I have a 'string' named 'string'

When I exit with error message 'string'
EOF
    run $ZENROOM_EXECUTABLE -z -a when_error.data.json when_error.zen
    assert_failure
