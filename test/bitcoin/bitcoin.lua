...
function bigFromString(str)
   ...
   for i=1, #str, 1 do
      local digit = tonumber(str:sub(i,i), 10)
      if digit == nil then
...
sk = readWIFPrivateKey('cRg4MM15LCfvt4oCddAfUgWm54hXw1LFmkHqs6pwym9QopG5Evpt')
pk = ECDH.pubgen(sk)
tx = {
   version=2,
   txIn = {
...
--tx.witness = buildWitness(tx, sk)
--rawTx = buildRawTransaction(tx)
-- In the chain f46e7234e143f51ba788ea4b7ae554691baa01dd9f68116626f92812836efe1f
...
-- unspent = {
--   {
--      txid= O.from_hex("5010412c2e0bfbeb5f0f277a9fea3b8053feb1b8411169e28ddb3ce9ac45212d"),
--      vout= 0,
--      address= "tb1q73czlxl7us4s6num5sjlnq6r0yuf8uh5clr2tm",
--      -- scriptPubKey= O.from_hex("0014f4702f9bfee42b0d4f9ba425f98343793893f2f4"),
--      amount= INT.new(O.from_hex('1d4430')),
--   }
-- }

-- tx = buildTxFromUnspent(unspent, sk, "tb1q04c9a079f3urc5nav647frx4x25hlv5vanfgug", INT.new(O.from_hex('0f4240')), INT.new(O.from_hex('03e8')))
-- tx.witness = buildWitness(tx, sk)
-- rawTx = buildRawTransaction(tx)
--print(rawTx:hex())

-- Sent on the chain e89bf0ac3e029cc95dddcec228f003a97ec0010c8eb5a88ae94ef5b14bf963da
...
rawTx = rawTransactionFromJSON(DATA, keys.private_key):hex()
print(rawTx)
