...
int zenroom_hash_update(const char *hash_ctx,
	...
	if (!hex_buf) {
		_err("%s :: cannot allocate hex_buf", __func__);
		return FAIL();
	}
	if (hex2buf(hex_buf, buffer) < 0) {
		free(hex_buf);
		_err("%s :: cannot do hex2buf %s", __func__, buffer);
		return FAIL();
	}
	if (prefix == ZEN_SHA512) {
		...
		if (hex2buf(sh, hash_ctx + 1) < 0) {
			free(sh);
			free(hex_buf);
			_err("%s :: cannot do hex2buf %s", __func__, hash_ctx + 1);
			return FAIL();
		}
...
int zenroom_hash_final(const char *hash_ctx) {
	...
	if (!hash_result) {
		_err("%s :: cannot allocate hash_result", __func__);
		return FAIL();
	}
	octet tmp;
	char *sh;
	if (prefix == ZEN_SHA512) {
		...
		if (!tmp.val) {
			free(hash_result);
			_err("%s :: cannot allocate tmp.val", __func__);
			return FAIL();
		}
		len = sizeof(hash512);
