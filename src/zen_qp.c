...
extern int PQCP_MLKEM_NATIVE_MLKEM512_keypair_derand(uint8_t *pk, uint8_t *sk, const uint8_t *coins);
extern int PQCP_MLKEM_NATIVE_MLKEM512_keypair(uint8_t *pk, uint8_t *sk);
extern int PQCP_MLKEM_NATIVE_MLKEM512_enc_derand(uint8_t *ct, uint8_t *ss, const uint8_t *pk, const uint8_t *coins);
...
static int qp_ml_kem_512_keygen(lua_State *L) {
	...
	lua_createtable(L, 0, 2);
	octet *private = o_new(L, PQCP_MLKEM_NATIVE_MLKEM512_SECRETKEYBYTES);
	lua_setfield(L, -2, "private");
	octet *public = o_new(L, PQCP_MLKEM_NATIVE_MLKEM512_PUBLICKEYBYTES);
	lua_setfield(L, -2, "public");

	PQCP_MLKEM_NATIVE_MLKEM512_keypair_derand((unsigned char*)public->val, (unsigned char*)private->val, randbytes);
	public->len = PQCP_MLKEM_NATIVE_MLKEM512_PUBLICKEYBYTES;
	private->len = PQCP_MLKEM_NATIVE_MLKEM512_SECRETKEYBYTES;
end:
	if(failed_msg) {
		THROW(failed_msg);
	}
...
static int qp_ml_kem_512_pubgen(lua_State *L) {
	...
	pk = o_new(L, PQCP_MLKEM_NATIVE_MLKEM512_PUBLICKEYBYTES);
	if(pk == NULL) {
		failed_msg = "Could not allocate private key";
		goto end;
	}
	...
	memcpy((unsigned char*)pk->val,
				(unsigned char*)sk->val + pqcrystal_mlkem_512_INDCPA_SECRETKEYBYTES, PQCP_MLKEM_NATIVE_MLKEM512_PUBLICKEYBYTES);
	pk->len = PQCLEAN_KYBER512_CLEAN_CRYPTO_PUBLICKEYBYTES;
...
static int qp_ml_kem_512_sscheck(lua_State *L) {
	...
	if(ss == NULL) {
		...
		if(ss->len == PQCP_MLKEM_NATIVE_MLKEM512_BYTES)
			lua_pushboolean(L, 1);
		else
			lua_pushboolean(L, 0);
		o_free(L, ss);
...
static int qp_ml_kem_512_ctcheck(lua_State *L) {
	...
	if(ct == NULL) {
		...
		if(ct->len == PQCP_MLKEM_NATIVE_MLKEM512_CIPHERTEXTBYTES)
			lua_pushboolean(L, 1);
		else
			lua_pushboolean(L, 0);
		o_free(L, ct);
...
static int qp_ml_kem_512_enc(lua_State *L) {
	...
	if(pk->len != PQCP_MLKEM_NATIVE_MLKEM512_PUBLICKEYBYTES) {
		failed_msg = "invalid size for public key";
		goto end;
	}
	lua_createtable(L, 0, 2);
	...
	lua_setfield(L, -2, "secret"); // shared secret
	ct = o_new(L, PQCP_MLKEM_NATIVE_MLKEM512_CIPHERTEXTBYTES);
	if(ct == NULL) {
		failed_msg = "Could not allocate kem ciphertext";
		goto end;
	}
	...
	if(PQCP_MLKEM_NATIVE_MLKEM512_enc_derand((unsigned char*)ct->val,
						 (unsigned char*)ss->val,
						 (unsigned char*)pk->val,
						 randbytes)) {
		failed_msg = "error in the creation of the shared secret";
	...
	ss->len = KYBER_SSBYTES;
	ct->len = PQCP_MLKEM_NATIVE_MLKEM512_CIPHERTEXTBYTES;
end:
	o_free(L,pk);
	if(failed_msg) {
		THROW(failed_msg);
...
static int qp_ml_kem_512_dec(lua_State *L) {
	...
	if(sk->len != PQCP_MLKEM_NATIVE_MLKEM512_SECRETKEYBYTES) {
		failed_msg = "invalid size for secret key";
		goto end;
	}
	if(ct->len != PQCP_MLKEM_NATIVE_MLKEM512_CIPHERTEXTBYTES) {
	...
	if(PQCP_MLKEM_NATIVE_MLKEM512_dec((unsigned char*)ss->val,
						 (unsigned char*)ct->val,
						 (unsigned char*)sk->val)) {
		failed_msg = "error in while deciphering the shared secret";
		goto end;
