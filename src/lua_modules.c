...
int zen_require_restricted(lua_State *L) {
	SAFE(L);
	size_t len;
	...
	for (p = zen_extensions;
	     p->name != NULL; ++p) {
		// restrictions
		if (strncasecmp(p->name, "init", 4) == 0) continue;
		if (strncasecmp(p->name, "ast", 3) == 0) continue;
...
int zen_require_override(lua_State *L, const int restricted) {
	...
	static const struct luaL_Reg custom_require_restricted [] =
		{ {"require", zen_require_restricted },
		  {NULL, NULL} };
...
void zen_load_extensions(lua_State *L) {
	...
	// second argument is restricted flag
