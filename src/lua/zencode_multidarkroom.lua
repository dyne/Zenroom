...
local function import_multidarkroom_session_fingerprints_f(o)
    ...
    local arr = {}
    for _, v in ipairs(rawarr) do table.insert(arr, ECP.new(v)) end
    return arr
end
...
ZEN.add_schema({
    ...
    multidarkroom_session = function(obj)
        return {
            ...
            fingerprints = import_multidarkroom_session_fingerprints_f(obj.fingerprints)
        }
    end,
...
When("create the bls key", function()
    -- keygen: δ = r.O ; γ = δ.G2
    initkeys'bls'
    ACK.keys.bls = INT.random() -- BLS secret signing key
end)

When("create the bls public key", function()
...
When("verify the multidarkroom signature credential", function()
    have 'multidarkroom_signature'
    have 'verifiers'
    have 'multidarkroom_session'
    ZEN.assert(ABC.verify_cred_uid(ACK.verifiers, ACK.multidarkroom_signature.proof,
                                   ACK.multidarkroom_signature.zeta, ACK.multidarkroom_session.UID),
...
When("check the multidarkroom signature fingerprint is new", function()
    have 'multidarkroom_signature'
    have 'multidarkroom_session'
    if not ACK.multidarkroom_session.fingerprints then return end
    ZEN.assert(not ACK.multidarkroom_session.fingerprints[ACK.multidarkroom_signature.zeta],
               "Signature fingerprint is not new")
end)

When("add the multidarkroom fingerprint to the multidarkroom session", function()
    have 'multidarkroom_signature'
    have 'multidarkroom_session'
    if not ACK.multidarkroom_session.fingerprints then
        ACK.multidarkroom_session.fingerprints = { ACK.multidarkroom_signature.zeta }
    else
        table.insert(ACK.multidarkroom_session.fingerprints, ACK.multidarkroom_signature.zeta)
    end
end)

When("add the multidarkroom signature to the multidarkroom session", function()
    have 'multidarkroom_session'
    have 'multidarkroom_signature'
    ACK.multidarkroom_session.SM = ACK.multidarkroom_session.SM + ACK.multidarkroom_signature.signature
end)

When("verify the multidarkroom session is valid", function()
    have 'multidarkroom_session'
    ZEN.assert(
        ECP2.miller(ACK.multidarkroom_session.verifier, ACK.multidarkroom_session.UID) ==
