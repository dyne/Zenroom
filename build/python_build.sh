...
# to prepare the python build environment on Devuan:
# setup the build user (jenkins)
...
# 3.5.0 3.5.1 3.5.2 removed for incompatibility with openssl-1.1
pys=(3.5.3 3.5.4 3.5.5 3.5.6)
...
command -v pyenv > /dev/null && {
	...
	cp -v "src/_zenroom.so" $PYTHON_PACKAGE
	echo
	done
}
