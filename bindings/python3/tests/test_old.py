import json
import pytest
from zenroom import lua_call, zencode_call

LOAD_FATIGUE = 20


def test_basic(apply_with_process):
    script = "print('Hello world')"
    res = apply_with_process(
        lua_call,
        script
    )
    assert "Hello world" == res.output


@pytest.mark.skip
def test_keygen(): # XXX map function not in scope
    # script = """
...
@pytest.mark.skip  # XXX Wont execute in apiroom
def test_zencode(apply_with_process):
...
Then print all data
"""
    result = apply_with_process(
        zencode_call,
        contract
    )
    assert result.json.get("public")
    assert result.json.get("private")
...
@pytest.mark.skip
def test_load_test():  # XXX Won't execute in apiroom
...
@pytest.mark.skip
def test_load_script():  # XXX I assume it won't execute (see above)
...
def test_data(apply_with_process):
    script = "print(DATA)"
    data = "3"
...
@pytest.mark.skip
def st_tally_count():  # XXX Won't execute in apiroom
