...
const LEGACY_HASH_API_WARNING =
  "[DEPRECATED] The legacy direct hash API (`zenroom_hash_init`, `zenroom_hash_update`, `zenroom_hash_final`, `zenroom_hash`) is deprecated. Use the new direct hex API (`zenroom_hash_hex_tobuf` / `zenroom_hash_hex`, or `hashHex` in JavaScript).\n";
...
test("Run hash api", async (t) => {
  ...
  t.is(ctx.logs, LEGACY_HASH_API_WARNING);
  t.is(
    ctx.result,
...
test("Use zenroom_hash with small input", async (t) => {
  ...
  t.true(hash.logs.includes(LEGACY_HASH_API_WARNING), hash.logs);
  t.is(
    hash.result,
    "wAkmo3TN5VuPvXf1DaE2PaGXRNP0ZOB84xeUxaYbb5yFaJ+hz+E2VTUn/Ydr6RZzwsrC3RV7Le/qNghRttks9A=="
  );
