-- use case scenario:
-- intrspection.json --> contains all the statements present in src/lua/zencode_*.lua
...
local function all_statements(file_name)
   ...
   while true do
	 ...
	 scenario = io.read()
	 if scenario:sub(-1) == "]" then
	    scenario = scenario:sub(1, -5)
	 else
	    scenario = scenario:sub(1, -4) 
	 end
	 -- saving it
	 d[scenario] = {}
	 table.insert(scenarios, scenario) -- used to create a sorted output file
      elseif line ~= "]" and line ~= "}" and line ~= "\"\"" then
	 d[scenario][line:sub(1, -2):lower()] = true -- saving the statement
      end
   end
   io.input():close()
